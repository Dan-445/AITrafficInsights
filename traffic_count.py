...
def process_data(new_data):
    ...
    for key in ['car', 'motorcycle', 'person', 'truck']:
...
def save_to_excel_with_timestamps(new_data):
    ...
    PV = pd.DataFrame(vehicle_data.get('car'))
    PV = PV.drop(columns=["prevRawCount"])
    
    TTST = pd.DataFrame(vehicle_data.get('motorcycle'))
    TTST = TTST.drop(columns=["prevRawCount"])
    
    Duals = pd.DataFrame(vehicle_data.get('person'))
    Duals = Duals.drop(columns=["prevRawCount"])

    Twins = pd.DataFrame(vehicle_data.get('truck'))
    Twins = Twins.drop(columns=["prevRawCount"])

    # Save DataFrame to Excel
    with pd.ExcelWriter(output_excel_path) as writer:
        PV.to_excel(writer, sheet_name='car', index=False)
        TTST.to_excel(writer, sheet_name='motorcycle', index=False)
        Duals.to_excel(writer, sheet_name='person', index=False)
        Twins.to_excel(writer, sheet_name='truck', index=False)

    start_time = (datetime.fromtimestamp(start_time) + timedelta(seconds=interval)).timestamp()

def process_video(video_path, output_path):
    cap = cv2.VideoCapture(video_path)
    #cap.set(cv2.CAP_PROP_POS_FRAMES, 900)#yekeun
   ...
   # class_names = model.names
    ...
    while cap.isOpened():
        ...
        if tracks:  # Ensure tracks are not empty or None
            ...
            for index, key in enumerate(['car', 'motorcycle', 'person', 'truck']):
        ...
        print(elapsed_time,interval, "time check")
...
for video_file in os.listdir(video_dir):
    if video_file.endswith('.mp4') or video_file.endswith('.MP4'):  # Adjust the file extension if needed
        print(video_file)
