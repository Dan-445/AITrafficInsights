...
def process_data(new_data):
    ...
    for key in ['car', 'bicycle', 'person', 'truck']:
...
def save_to_excel_with_timestamps(new_data):
    ...
    TTST = pd.DataFrame(vehicle_data.get('bicycle'))
    ...
    with pd.ExcelWriter(output_excel_path) as writer:
        ...
        TTST.to_excel(writer, sheet_name='bicycle', index=False)
...
def process_video(video_path, output_path):
    ...
    while cap.isOpened():
        ...
        if tracks:  # Ensure tracks are not empty or None
            ...
            for index, key in enumerate(['car', 'bicycle', 'person', 'truck']):
         ...
         #   if cap.get(cv2.CAP_PROP_POS_FRAMES) >= 900:
...
video_dir = "videos"
output_excel_path = 'sb_traffic_count_results.xlsx'
